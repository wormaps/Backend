...
export class SceneHeroOverrideMatcherService {
  ...
  findMatchingLandmarkBuilding(
    ...
    const exact = annotation.objectId
      ? (meta.buildings.find(
          (building) => building.objectId === annotation.objectId,
        ) ?? null)
      : null;
    if (exact) {
      return exact;
  ...
  resolveLandmarkAssignments(
    ...
    for (const annotation of manifest.landmarks.filter(
      (item) => item.kind === 'BUILDING',
    )) {
      const matched = this.findPreferredBuilding(
        meta,
        annotation,
        usedBuildings,
      );
